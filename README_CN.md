...
本项目基于 **YukiHookAPI 1.3.2** 实现（自 v0.0.10 起全面重构）。模块入口使用 `@InjectYukiHookWithXposed` 注解，由 KSP 处理器自动生成 Xposed 入口资产（`assets/xposed_init`）与模块状态检测类。
...
### 跨进程偏好设置（YukiHookPrefsBridge）

模块应用与 system_server 之间通过 YukiHookAPI 的 `YukiHookPrefsBridge` 共享同一份偏好数据：
- 模块应用内：`context.prefs()` 可读可写；
- system_server（宿主进程）内：通过 XSharedPreferences 只读同一文件。

因此应用里设置的 MAC、开关会**实时**作用于 Hook 逻辑，无需自建广播桥接。

### 激活状态检测（YukiHookAPI.Status）

应用内通过 `YukiHookAPI.Status.isModuleActive` 判断模块是否已在 LSPosed 中激活。LSPosed 会自动向模块自身进程注入真实的激活状态，重启后进入应用**立即显示正确状态**，不再依赖“是否收到系统广播”等瞬态信号。

### 出厂 MAC 获取（系统 MAC 显示）

模块反射 `WifiVendorHal.getStaFactoryMacAddress(iface)`（ColorOS/OPPO 方法名；AOSP 标准为 `getFactoryMacAddress`，已做多候选兼容）获取**硬件出厂 MAC**，不受 MAC 随机化与模块替换影响。应用进程启动时通过 **YukiHookAPI 的 `YukiHookDataChannel`** 向 system_server 主动拉取并缓存，**打开界面立即显示系统 MAC**（无需等待 WiFi 广播）。
